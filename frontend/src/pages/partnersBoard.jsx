    ...
    <Card>
      <CardHeader>
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-lg font-semibold text-gray-800 dark:text-text-light">Our Top Partners</h3>
        </div>
      </CardHeader>
      <CardContent>
        <ul className="divide-y divide-gray-200 dark:divide-gray-700">
          {partners.map((partner, index) => (
            <li key={index} className="py-3 flex justify-between items-center">
              <span className="text-gray-800 dark:text-text-light font-medium">{partner.name}</span>
              <span className="text-sm text-gray-500 dark:text-text-muted">{partner.contribution}</span>
            </li>
          ))}
        </ul>
      </CardContent>
