...
export default function generateReport(req, res) {
  ...
  db.query(query, params)
    .then(result => {
      ...
      const pollutedCount = locationsData.filter(loc => loc.classification === 'Polluted').length;
      const highlyPollutedCount = locationsData.filter(loc => loc.classification === 'Highly Polluted').length;
      ...
      locationsData.forEach(loc => {
        ...
        doc.fontSize(16).font('Helvetica-Bold').fillColor(COLORS.TEXT_DARK).text(loc.location, 65, currentY + 15);
                doc.fontSize(10).font('Helvetica').fillColor(COLORS.TEXT_MUTED)
           .text(`Samples Analyzed: ${loc.count}`, 65, currentY + 38);

        doc.fillColor(classificationColor)
           .roundedRect(doc.page.width - 225, currentY + 15, 160, 25, 5)
           .fill();
        doc.fillColor('#FFF').fontSize(11).font('Helvetica-Bold')
           .text(loc.classification.toUpperCase(), doc.page.width - 225, currentY + 22, {
             width: 160,
             align: 'center'
           });
